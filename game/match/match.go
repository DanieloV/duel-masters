...
func (m *Match) MoveCard(card *Card, destination string, source *Card) {
	...
	m.ReportActionInChat(card.Player, fmt.Sprintf("%s was moved to %s %s by %s", m.FormatDisplayableCard(card), card.Player.Username(), destination, m.FormatDisplayableCard(source)))
...
// NewDrawCountAction prompts the user to select a number of cards to draw
func (m *Match) NewDrawCountAction(player *Player, text string, minSelections int, maxSelections int) {

	msg := &server.ActionMessage{
		Header:        "action",
		ActionType:    "drawCount",
		Text:          text,
		MinSelections: minSelections,
		MaxSelections: maxSelections,
	}
...
func (m *Match) ChargeMana(p *PlayerReference, cardID string) {
	...
	if card, err := p.Player.MoveCard(cardID, HAND, MANAZONE, cardID); err == nil {
		...
		m.BroadcastState()
		m.ReportActionInChat(p.Player, fmt.Sprintf("%s was added to %s's manazone",
