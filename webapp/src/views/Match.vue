<template>
  ...
  >
    ...
    <div v-if="action" id="action" class="action noselect">
      ...
      <template v-if="action.actionType == 'drawCount'">
        <div class="flex justify-center mb-4">
          <img
            class="h-8"
            :class="{'inactive-svg-button' : actionDrawCount == action.minSelections}"
            @click="modifyActionDrawCount(-1)"
            src="/assets/images/left-arrow.svg"
          />
          <input
            class="ml-2 mr-2 w-4 bg-white pointer-events-none text-center" 
            v-model.number="actionDrawCount" 
            :disabled="true"
          />
          <img
            class="h-8"
            :class="{'inactive-svg-button' : actionDrawCount == action.maxSelections}"
            @click="modifyActionDrawCount(1)"
            src="/assets/images/right-arrow.svg"
          />
        </div>
        <div @click="chooseDrawAction()" class="btn">Draw {{ actionDrawCount }}</div>
      </template>
...
export default {
  ...
  methods: {
    ...
    chooseDrawAction() {
      ...
      this.ws.send(JSON.stringify({ header: "action", drawCount: this.actionDrawCount, cancel: false }));
    },

    modifyActionDrawCount(modifier) {
      var newCount = this.actionDrawCount + modifier
      if (newCount > this.action.maxSelections || newCount < this.action.minSelections) {
        // Play sound for reaching limit here if needed
        return;
      }  
      this.actionDrawCount = newCount 
    },
        ...
        switch (data.header) {
          ...
          case "action": {
            ...
            switch (data.actionType) {
              case 'drawCount':
                this.actionDrawCount = data.maxSelections
                this.action = {
                  text: data.text,
